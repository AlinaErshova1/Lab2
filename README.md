...
2.Фиксируя `Z`, перемножаем положительные элементы каждого слоя, извлекаем корень степени `X` * `Y`  (т.к. элементов на одном слое `X` * `Y` шт.)  и таким образом находим сред. геом. выводим его.
...
Программа будет выводить `Z`  неотрицательных вещ. чисел - среднее геометрическое.
...
| Число 1 | Неотрицательное вещественное число |0|1.79<sup>308</sup>|
| ... | ... |...|...|
| Число Z | Неотрицательное вещественное число |0|1.79<sup>308</sup>|
| Mассив                                                            |
...
Формула среднего геометрического - корень из произведения положительных чисел - (x<sub> 1 </sub> * x<sub> 2 </sub> * ... * x<sub> n </sub> )<sup> 1/n </sup>
...
3.2 Затем нахожу произведение положительных чисел находящихся на 0-ом слое. И вычисляю корень степени (`х` * `у` ) из получившегося произведения. И вывожy результат. И т.д. для отставшихся слоёв.
